...
public static class ImGuiRenderer
{
    ...
    private static Dictionary<KeyboardKey, ImGuiKey> _raylibKeyMap = new();
    ...
    public static void Setup(bool darkTheme = false, bool enableDocking = false)
    {
        BeginInitImGui();
    ...
    public static unsafe void ReloadFonts()
    {
        ...
        var raylibData = Raylib.MemAlloc((uint)data.Length);
        Marshal.Copy(data, 0, (IntPtr)raylibData, data.Length);
        var image = new Image
        {
            Data = raylibData,
            Width = width,
